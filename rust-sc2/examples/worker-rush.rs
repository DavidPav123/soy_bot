...
impl Player for WorkerRushAI {
    ...
    fn on_step(&mut self, _iteration: usize) -> SC2Result<()> {
        let ground_attackers = self.units.enemy.units.filter(|u| {
            !u.is_flying() && u.can_attack_ground() && u.is_closer(45.0, self.enemy_start)
        });
        if !ground_attackers.is_empty() {
            ...
            if !ground_structures.is_empty() {
                for u in &self.units.my.workers {
                    u.attack(
                        Target::Tag(ground_structures.closest(u).unwrap().tag()),
                        false,
                    );
                }
            } else {
