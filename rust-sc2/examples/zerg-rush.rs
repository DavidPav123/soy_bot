...
impl ZergRushAI {
    ...
    fn distribute_workers(&mut self) {
        ...
        if idle_workers.is_empty()
            && *last_loop + Self::DISTRIBUTION_DELAY + bases.len() as u32 > game_loop
        {
            return;
        }
        *last_loop = game_loop;
    ...
    fn order_units(&mut self) {
        ...
        if self.counter().all().count(queen) < self.units.my.townhalls.len()
        ...
        if (self.supply_workers as usize)
            < 96.min(self.counter().all().count(UnitTypeId::Hatchery) * 16)
