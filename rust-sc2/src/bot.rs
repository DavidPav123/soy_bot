...
impl<'a> CountOptions<'a> {
    ...
    pub fn count(&self, unit_id: UnitTypeId) -> usize {
        ...
        match self.alias {
            ...
            UnitAlias::Unit => {
                count(unit_id) + UNIT_ALIAS.get(&unit_id).copied().map(count).unwrap_or(0)
            }
            UnitAlias::Tech => {
                count(unit_id)
                    + TECH_ALIAS
...
impl Bot {
    ...
    */
    ...
    pub(crate) fn update_units(&mut self, all_units: Units) {
        *self.last_units_health.write_lock() = self
        ...
        for exp in &mut self.expansions {
            let (alliance, base) = expansions
                .remove(&exp.loc)
                .unwrap_or((Alliance::Neutral, None));
    ...
    pub fn can_place_some(&self, places: Vec<(UnitTypeId, Point2)>) -> Vec<bool> {
        self.query_placement(
            places
                .into_iter()
                .map(|(building, pos)| {
                    (self.game_data.units[&building].ability.unwrap(), pos, None)
                })
                .collect(),
            false,
        )
    ...
    pub fn find_placement(
        ...
        if let Some(data) = self.game_data.units.get(&building) {
            if let Some(ability) = data.ability {
                ...
                for distance in
                    (placement_step..options.max_distance).step_by(placement_step as usize)
                {
    ...
    pub fn free_expansions(&self) -> impl Iterator<Item = &Expansion> {
        self.expansions
            .iter()
            .filter(|exp| exp.alliance.is_neutral())
    }
