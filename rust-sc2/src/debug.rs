...
use sc2_proto::debug::{
    ...
    DebugEndGame_EndResult, DebugGameState as ProtoDebugGameState, DebugLine,
    DebugSetUnitValue_UnitValue, DebugSphere, DebugText,
};
...
impl Debugger {
    ...
    pub fn draw_text_screen(
        ...
        self.draw_text(
            text,
            DebugPos::Screen(pos.unwrap_or((0.0, 0.0))),
            color,
            size,
        );
    }
    ...
    pub fn draw_sphere(&mut self, pos: Point3, radius: f32, color: Option<Color>) {
        self.debug_drawings
            .push(DebugDraw::Sphere(pos, radius, color));
    }
    ...
    {
        self.debug_commands.extend(cmds.into_iter().copied().map(
            |(type_id, owner, pos, count)| DebugCommand::CreateUnit(type_id, owner, pos, count),
        ));
    }
