    ...
    B: Player + DerefMut<Target = Bot> + Deref<Target = Bot>,
{
    ...
    raw.effects = res_raw
        ...
        .map(|e| Effect {
            id: {
                ...
                EffectId::from_u32(id)
                    .unwrap_or_else(|| panic!("There's no `EffectId` with value {}", id))
            },
    ...
    for u in &dead_units {
        let alliance = if bot.owned_tags.remove(u) {
            ...
            if removed {
                ...
                .expansions
                .iter_mut()
                .any(|exp| exp.minerals.swap_remove(u))
            {
    ...
    *raw.upgrades.write_lock() = raw_player
        ...
        .iter()
        .map(|u| {
            UpgradeId::from_u32(*u)
                .unwrap_or_else(|| panic!("There's no `UpgradeId` with value {}", u))
        })
        .collect::<FxHashSet<_>>();
