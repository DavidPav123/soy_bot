...
impl Unit {
    ...
    pub fn building_size(&self) -> Option<usize> {
        if self.is_addon() {
            ...
            self.footprint_radius()
                .map(|radius| (radius * 2.0) as usize)
        }
    }
    ...
    pub fn target(&self) -> Target {
        self.orders()
            .first()
            .map_or(Target::None, |order| order.target)
    }
    ...
    pub fn is_almost_unused(&self) -> bool {
        if self.has_reactor() {
            self.orders().len() < 2
                || (self.orders().len() == 2
                    && self.orders().iter().any(|order| order.progress >= 0.95))
        } else {
            self.is_almost_idle()
        }
    }
        ...
        Self {
            ...
            base: Rs::new(UnitBase {
                ...
                buffs: u
                    ...
                    .map(|b| {
                        BuffId::from_u32(*b)
                            .unwrap_or_else(|| panic!("There's no `BuffId` with value {}", b))
                    })
                    .collect(),
                ...
                orders: u
                    ...
                    .map(|order| UnitOrder {
                        ability: {
                            ...
                            AbilityId::from_u32(id).unwrap_or_else(|| {
                ...
                passengers: u
                    ...
                    .map(|p| PassengerUnit {
                        ...
                        type_id: {
                            ...
                            UnitTypeId::from_u32(id).unwrap_or_else(|| {
                                panic!("There's no `UnitTypeId` with value {}", id)
                            })
                        },
                    })
                    .collect(),
