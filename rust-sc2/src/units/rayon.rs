...
impl Units {
    ...
    pub fn closest_distance_squared<P: Into<Point2> + Copy + Sync>(
        &self,
        target: P,
    ) -> Option<f32> {
        self.min_value(|u| u.distance_squared(target))
    ...
    pub fn furthest_distance_squared<P: Into<Point2> + Copy + Sync>(
        &self,
        target: P,
    ) -> Option<f32> {
        self.max_value(|u| u.distance_squared(target))
...
impl ParallelExtend<Unit> for Units {
    ...
    fn par_extend<T: IntoParallelIterator<Item = Unit>>(&mut self, par_iter: T) {
        self.0
            .par_extend(par_iter.into_par_iter().map(|u| (u.tag(), u)));
    }
}
    ...
    Self::Item: Borrow<Unit>,
{
    ...
    fn find_tags<T: Container<u64>>(self, tags: &'_ T) -> FindTags<'_, Self, T> {
    ...
    fn exclude_types<T: Container<UnitTypeId>>(self, types: &'_ T) -> ExcludeTypes<'_, Self, T> {
